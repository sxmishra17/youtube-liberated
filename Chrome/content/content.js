 ...
 *
 * Key Operations:
 * 1. Read toggle states from chrome storage (ads, shorts, posts, bg playback).
 * 2. Toggle helper classes on the root <html> element to apply CSS hiding rules.
 * 3. Set a custom dataset attribute on the root tag to share background playback status with inject.js.
 ...
 *   4. Browse to the extension folder and select the folder containing "manifest.json".
 ...
 *   - Use the "Background Play" toggle to enable/disable background audio playback.
 *   - All toggles default to ON. Changes take effect instantly (no reload).
...
(function () {
  ...
  // Update root element HTML classes and attributes based on current active configs
  function updateHtmlClasses() {
    ...
    if (blockGamesActive) {
      root.classList.add('yt-block-games');
    } else {
      root.classList.remove('yt-block-games');
    }
  }
  ...
  function initializeSettings() {
    ...
    extensionStorage.get(defaultSettings, (items) => {
      ...
      blockGamesActive = (items && items.blockGames !== undefined) ? items.blockGames : true;
  ...
  function onSettingsChanged(changes, areaName) {
    ...
    if (changes.blockGames) {
      blockGamesActive = changes.blockGames.newValue;
    }
   ...
   * Updates a dataset attribute on the html tag which the main-world inject.js
   * script checks. This toggle controls whether background play logic is active.
   */
  function updateBackgroundPlayback() {
    const root = document.documentElement;
    if (root) {
      root.dataset.bgPlaybackActive = bgPlaybackActive.toString();
    }
  }
